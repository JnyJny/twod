...
def test_crect_add_empty_point():
    p = Point()
    r = Rect(1, 2, 3, 4)
    q = r + p
...
def test_crect_add_nonempty_point():
    r = Rect(1, 2, 3, 4)
    p = Point(2, 3)
    q = r + p
...
def test_crect_iadd_empty_point():
    r = Rect(1, 2, 3, 4)
    p = Point()
    r += p
...
def test_crect_iadd_nonempty_point():
...
def test_crect_iadd_empty_rect():
    r = Rect(1, 2, 3, 4)
    s = Rect()
    r += s
    assert r.x == 1 and r.y == 2 and r.w == 3 and r.h == 4
